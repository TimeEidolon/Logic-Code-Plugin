...
当前文件夹是可分享的 Skill 包，核心 Skill 文件是：

`SKILL.md`

如果要让 Cursor 自动发现该 Skill，需要把整个文件夹复制或软链接到 Cursor 支持的位置：

```text
~/.cursor/skills/java-project-report/
```

或某个项目内：

```text
<project>/.cursor/skills/java-project-report/
```

后续可以直接对 Cursor 说：

```text
使用 java-project-report skill，分析 /path/to/java/project，并把最终报告保存到 FunctionTree
```
...
```text
<target-project>/PROJECT_ANALYSIS_REPORT.md
<target-project>/PROJECT_EXCEPTION_PLAYBOOK.md
FunctionTree/<project-name>-analysis-summary.md
```

如果需要可视化概览，再生成：

```text
<cursor-project>/canvases/<project-name>-analysis.canvas.tsx
```
...
`java_project_inventory.py`

使用方式：

```bash
python3 java_project_inventory.py /path/to/java/project
```

该脚本只做静态盘点，不替代人工报告生成。它适合在正式分析前快速列出 Maven 模块、Controller、Service、配置文件和关键注解位置。
...
```python
generate_create(project_path, output_dir=None, call_model=False)
```

默认模式不调用模型，只生成：

- `PROJECT_INVENTORY.md`
- `PROJECT_SOURCE_CONTEXT.md`
- `GENERATE_REPORT_PROMPT.md`

使用方式：

```bash
python3 generate_report.py /path/to/java/project
```
...
如果要直接调用 OpenAI-compatible 大模型生成最终报告：
...
python3 generate_report.py /path/to/java/project --call-model
```
